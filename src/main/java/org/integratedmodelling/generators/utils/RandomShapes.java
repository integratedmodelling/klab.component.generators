...
public class RandomShapes {
    ...
    /**
     * Create a number of random boxes by subdividing the sides of an envelope into a specified number of
     * divisions (approximate), then put a random shape in each with a specified number of vertices and
     * probability. The resulting shapes may be of different area but will never overlap.
     *
     * @param envelope
     * @param xdivs
     * @param ydivs
     ...
     * @return
     */
    public Collection<Shape> create(Envelope envelope, int xdivs, int ydivs, double frequency, int vertices) {
        ...
        if (xdivs == 1 || ydivs == 1) {
             ...
             */
            ret.add(createPolygon(envelope.getMinX(), envelope.getMaxX(), envelope.getMinY(),
                                  envelope.getMaxY(), vertices, envelope.getProjection()));

        } else {

            List<Double> xbreaks = new ArrayList<>();
            List<Double> ybreaks = new ArrayList<>();
            for (double xlimit = envelope.getMinX(); xlimit < envelope.getMaxX(); xlimit += randomIncrement(
                    xwidth, xnormal)) {
                xbreaks.add(xlimit);
            }
            for (double ylimit = envelope.getMinY(); ylimit < envelope.getMaxY(); ylimit += randomIncrement(
                    ywidth, ynormal)) {
                ybreaks.add(ylimit);
            }
            ...
            for (int x = 1; x < xbreaks.size(); x++) {
                for (int y = 1; y < ybreaks.size(); y++) {
                    if (Math.random() <= frequency) {
                        ret.add(createPolygon(xbreaks.get(x - 1), xbreaks.get(x), ybreaks.get(y - 1),
                                              ybreaks.get(y), vertices, envelope.getProjection()));
                    }
                }
    ...
    private Shape createPolygon(double x0, double x1, double y0, double y1, int vertices,
    ...
    //
    //        String tzShape = "EPSG:4326 POLYGON((33.796 -7.086, 35.946 -7.086, 35.946 -9.41, 33.796 -9
    //        .41, 33.796 -7.086))";
